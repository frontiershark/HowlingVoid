	...
	switch(action)
		if("open")
			if(door.locked)
				door.unbolt()
			door.open()
			playsound(door, '../assets/sound/machines/ping.ogg', 50, FALSE, SILENCED_SOUND_EXTRARANGE, ignore_walls = FALSE)
		...
		if("shock")
			...
			playsound(door, '../assets/sound/machines/buzz/buzz-sigh.ogg', 25, FALSE, SILENCED_SOUND_EXTRARANGE, ignore_walls = FALSE)
