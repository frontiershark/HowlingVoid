...
/datum/preference/choiced/glasses/icon_for(value)
	if (value == "Random")
		...
		return uni_icon(selected_type::icon, selected_type::icon_state) // NOVA EDIT CHANGE - Original: return uni_icon('../assets/icons/obj/clothing/glasses.dmi', "glasses_[LOWER_TEXT(value)]")

/datum/preference/choiced/glasses/is_accessible(datum/preferences/preferences)
	if (!..(preferences))
		return FALSE

	return "Nearsighted(Близорукий)" in preferences.all_quirks
