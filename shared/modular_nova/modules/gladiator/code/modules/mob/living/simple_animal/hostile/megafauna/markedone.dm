...
/mob/living/simple_animal/hostile/megafauna/gladiator/adjustHealth(amount, updating_health, forced)
	...
	if(prob(block_chance) && (phase == 1 || phase == 4) && !stunned)
		...
		playsound(src, '../assets/sound/items/weapons/parry.ogg', BLOCK_SOUND_VOLUME * 2, vary = TRUE) // louder because lavaland low pressure maybe?
		return FALSE
	. = ..()
	update_phase()
...
/mob/living/simple_animal/hostile/megafauna/gladiator/proc/bone_knife_throw(atom/target)
	...
	boned.throwforce = 35
	playsound(src, '../assets/sound/items/weapons/bolathrow.ogg', 60, 0)
