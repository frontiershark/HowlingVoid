...
/datum/preferences/proc/render_new_preview_appearance(mob/living/carbon/human/dummy/mannequin, show_job_clothes = TRUE)
	...
	if(preview_job)
		...
		if (istype(preview_job,/datum/job/ai))
			return image('../assets/icons/mob/silicon/ai.dmi', icon_state = resolve_ai_icon(read_preference(/datum/preference/choiced/ai_core_display)), dir = SOUTH)
		if (istype(preview_job,/datum/job/cyborg))
			return image('../assets/icons/mob/silicon/robots.dmi', icon_state = "robot", dir = SOUTH)
