...
/obj/machinery/computer/arcade/battle/proc/user_take_damage(mob/user, base_damage_taken)
	...
	if(player_current_hp <= 0)
		...
		playsound(loc, '../assets/sound/machines/arcade/lose.ogg', 40, TRUE)
		lose_game(user)
		return

	feedback_message = "User took [damage_taken] damage!"
	playsound(loc, '../assets/sound/machines/arcade/hit.ogg', 40, TRUE, extrarange = -3)
	SStgui.update_uis(src)
...
/obj/machinery/computer/arcade/battle/proc/process_player_attack(mob/user, attack_type)
	...
	switch(attack_type)
		...
		if(BATTLE_ARCADE_PLAYER_COUNTERATTACK)
			...
			playsound(loc, '../assets/sound/machines/arcade/mana.ogg', 40, TRUE, extrarange = -3)
...
/obj/machinery/computer/arcade/battle/proc/successful_counterattack(mob/user)
	...
	playsound(loc, '../assets/sound/machines/arcade/boom.ogg', 40, TRUE, extrarange = -3)
	if(enemy_hp <= 0)
		on_battle_win(user)
	SStgui.update_uis(src)
...
/obj/machinery/computer/arcade/battle/proc/perform_enemy_turn(mob/living/user, defending_flags = NONE)
	...
	if((enemy_hp != enemy_max_hp) && prob(chance_to_magic))
		if(enemy_mp >= 10)
			...
			playsound(loc, '../assets/sound/machines/arcade/heal.ogg', 40, TRUE, extrarange = -3)
			SStgui.update_uis(src)
			return
		if(player_current_mp >= 5) //minimum to steal
			...
			playsound(loc, '../assets/sound/machines/arcade/steal.ogg', 40, TRUE)
			SStgui.update_uis(src)
			return
		...
		if(UI_PANEL_SHOP)
			switch(action)
				if("sleep")
					...
					playsound(loc, '../assets/sound/vehicles/mecha/skyfall_power_up.ogg', 40)
			...
			switch(action)
				...
				if("continue_with_rest")
					if(prob(60))
						...
						playsound(loc, '../assets/sound/machines/defib/defib_zap.ogg', 40)
						if(prob(40))
