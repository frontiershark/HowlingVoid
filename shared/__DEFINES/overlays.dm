...
#define MAX_ATOM_OVERLAYS 120 // NOVA EDIT CHANGE - Taur parts mostly - ORIGINAL: #define MAX_ATOM_OVERLAYS 100
...
#define VALIDATE_OVERLAY_LIMIT(changed_on) \
	if(length(changed_on.overlays) >= MAX_ATOM_OVERLAYS) { \
		...
		changed_on.add_overlay(mutable_appearance('../assets/icons/testing/greyscale_error.dmi')); \
	} \
