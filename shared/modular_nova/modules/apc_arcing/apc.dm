		...
		switch(effect)
			if(SHOCK_SOMEONE)
				...
				if(length(shock_mobs))
					...
					playsound(get_turf(living_target), '../assets/sound/effects/magic/lightningshock.ogg', 75, TRUE)
					Beam(living_target, icon_state = "lightning[rand(1, 12)]", icon = '../assets/icons/effects/beam.dmi', time = 5)
					energy_fail(2)
			if(MAKE_SPARKS)
...
/obj/machinery/power/apc/proc/bronze_act(mob/living/user, obj/item/stack/sheet/bronze/bronze)
	...
	arc_shielded = TRUE
	playsound(src, '../assets/sound/items/tools/rped.ogg', 20)
