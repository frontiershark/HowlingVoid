...
/datum/component/scope/proc/zoom(mob/user)
	...
	if(HAS_TRAIT(user, TRAIT_USER_SCOPED))
		...
		return
	user.playsound_local(parent, '../assets/sound/items/weapons/scope.ogg', 75, TRUE)
...
/datum/component/scope/proc/stop_zooming(mob/user)
	...
	user.playsound_local(parent, '../assets/sound/items/weapons/scope.ogg', 75, TRUE, frequency = -1)
