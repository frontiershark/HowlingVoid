...
/datum/antagonist/traitor/submit_player_objective(retain_existing, retain_escape, force)
	...
	if (!.)
		return
	owner.current.playsound_local(get_turf(owner.current), '../assets/sound/music/antag/traitor/final_objective.ogg', 100, FALSE, pressure_affected = FALSE, use_reverb = FALSE)
...
/datum/antagonist/traitor/roundend_report()
	...
	if(uplink_owned)
		...
		if((used_telecrystals == 0) && traitor_won)
			var/static/icon/badass = icon('../assets/icons/ui/antags/badass.dmi', "badass")
