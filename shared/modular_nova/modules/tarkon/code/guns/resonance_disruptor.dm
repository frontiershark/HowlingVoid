...
/obj/item/ammo_casing/energy/resonance
	...
	fire_sound = '../assets/sound/items/weapons/kinetic_accel.ogg'
...
/datum/status_effect/resonant_link/on_apply()
	if(owner.mob_size >= MOB_SIZE_SMALL) // needs a minimal mass to resonate with.
		if(autodet)
			marked_underlay = mutable_appearance('../assets/icons/effects/effects.dmi', "shield2")
		else
			marked_underlay = mutable_appearance('../assets/icons/effects/effects.dmi', "shield1")
