...
/obj/item/lipstick/proc/display_radial_menu(mob/living/carbon/human/user)
	var/style_options = list(
		UPPER_LIP = icon('../assets/icons/hud/radial.dmi', UPPER_LIP),
		MIDDLE_LIP = icon('../assets/icons/hud/radial.dmi', MIDDLE_LIP),
		LOWER_LIP = icon('../assets/icons/hud/radial.dmi', LOWER_LIP),
	)
...
/obj/item/razor/proc/shave(mob/living/carbon/human/skinhead, location = BODY_ZONE_PRECISE_MOUTH)
	...
	playsound(loc, '../assets/sound/items/tools/welder2.ogg', 20, TRUE)
	...
	if(location == BODY_ZONE_PRECISE_MOUTH)
		if(!user.combat_mode)
			if(human_target.gender == MALE)
				...
				playsound(src, '../assets/sound/items/hair-clippers.ogg', 50)
