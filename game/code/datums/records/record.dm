...
/datum/record/crew/proc/make_photo(field_name, orientation, add_height_chart)
	...
	if(!isicon(character_appearance))
		...
		if(add_height_chart)
			appearance.underlays += mutable_appearance('../assets/icons/obj/machines/photobooth.dmi', "height_chart", alpha = 125, appearance_flags = RESET_ALPHA|RESET_COLOR|RESET_TRANSFORM|KEEP_APART)
