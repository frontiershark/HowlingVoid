...
/obj/machinery/requests_console/ui_act(action, list/params, datum/tgui/ui, datum/ui_state/state)
	...
	switch(action)
		...
		if("send_announcement")
			...
			minor_announce(message, "[department] Announcement:", html_encode = FALSE, sound_override = ANNOUNCER_DEPARTMENTAL) // NOVA EDIT CHANGE - Announcer Sounds - ORIGINAL: minor_announce(message, "[department] Announcement:", html_encode = FALSE, sound_override = '../assets/sound/announcer/announcement/announce_dig.ogg')
...
/obj/machinery/requests_console/proc/send_message(recipient, message, priority, request_type)
	...
	if(!silent)
		if(has_mail_send_error)
			...
			playsound(src, '../assets/sound/machines/beep/twobeep.ogg', 50, TRUE)
