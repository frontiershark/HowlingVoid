...
/datum/component/sign_language/proc/on_say(mob/living/carbon/carbon_parent, list/speech_args)
	...
	switch(emote_tone)
		if(TONE_INQUISITIVE)
			tonal_indicator = mutable_appearance('../assets/icons/mob/effects/talk.dmi', "signlang1", TYPING_LAYER)
		if(TONE_EMPHATIC)
			tonal_indicator = mutable_appearance('../assets/icons/mob/effects/talk.dmi', "signlang2", TYPING_LAYER)
