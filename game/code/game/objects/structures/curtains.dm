...
/obj/structure/curtain/attack_hand(mob/user, list/modifiers)
	...
	if(.)
		return
	playsound(loc, '../assets/sound/effects/curtain.ogg', 50, TRUE)
	toggle()
...
/obj/structure/curtain/play_attack_sound(damage_amount, damage_type = BRUTE, damage_flag = 0)
	switch(damage_type)
		if(BRUTE)
			if(damage_amount)
				playsound(src.loc, '../assets/sound/items/weapons/slash.ogg', 80, TRUE)
			else
				playsound(loc, '../assets/sound/items/weapons/tap.ogg', 50, TRUE)
		if(BURN)
			playsound(loc, '../assets/sound/items/tools/welder.ogg', 80, TRUE)
