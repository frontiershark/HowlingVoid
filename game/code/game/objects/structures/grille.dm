...
/obj/structure/grille/play_attack_sound(damage_amount, damage_type = BRUTE, damage_flag = 0)
	switch(damage_type)
		if(BRUTE)
			if(damage_amount)
				playsound(src, '../assets/sound/effects/grillehit.ogg', 80, TRUE)
			else
				playsound(src, '../assets/sound/items/weapons/tap.ogg', 50, TRUE)
		if(BURN)
			playsound(src, '../assets/sound/items/tools/welder.ogg', 80, TRUE)
	...
	if(isobj(AM))
		if(prob(50) && anchored && !broken)
			...
			if(O.throwforce != 0)//don't want to let people spam tesla bolts, this way it will break after time
				...
				if(C)
					playsound(src, '../assets/sound/effects/magic/lightningshock.ogg', 100, TRUE, extrarange = 5)
