...
/datum/disease/chronic_illness/stage_act(seconds_per_tick)
	...
	switch(stage)
		...
		if(4)
			var/need_mob_update = FALSE
			if(prob(30))
				affected_mob.playsound_local(affected_mob, '../assets/sound/effects/singlebeat.ogg', 100, FALSE, use_reverb = FALSE)
		...
		if(5)
			switch(rand(1,5)) // NOVA EDIT CHANGE - ORIGINAL: switch(rand(1,2))
				...
				if(2)
					...
					playsound(affected_mob.loc, '../assets/sound/effects/magic/repulse.ogg', 100, FALSE)
					affected_mob.emote("scream")
				...
				if(3)
					...
					playsound(src,'../assets/sound/effects/sparks/sparks4.ogg',50,TRUE)
					qdel(tunnel)
					update_stage(1)
				if(4)
				...
				if(5)
					...
					playsound(affected_mob, '../assets/sound/effects/hallucinations/far_noise.ogg', 50, 1)
					update_stage(1)
