...
/obj/narsie
	...
	icon = '../assets/icons/obj/antags/cult/narsie.dmi'
	icon_state = "narsie"
	anchored = TRUE
...
/obj/narsie/proc/start_ending_the_round()
	...
	sound_to_playing_players('../assets/sound/music/antag/bloodcult/narsie_rises.ogg')

	var/area/area = get_area(src)
	if(area)
		var/mutable_appearance/alert_overlay = mutable_appearance('../assets/icons/effects/cult.dmi', "ghostalertsie")
		notify_ghosts(
...
/obj/narsie/proc/fall_of_the_harbinger()
	...
	sound_to_playing_players('../assets/sound/effects/magic/demon_dies.ogg', 50)
...
/proc/narsie_end_begin_check()
	if(QDELETED(GLOB.cult_narsie)) // uno
		priority_announce("Status report? We detected an anomaly, but it disappeared almost immediately.","[command_name()] Higher Dimensional Affairs", '../assets/sound/announcer/notice/notice1.ogg')
		GLOB.cult_narsie = null
	...
	priority_announce(
		...
		sound = '../assets/sound/announcer/alarm/airraid.ogg',
	)
...
/proc/narsie_end_second_check()
	if(QDELETED(GLOB.cult_narsie)) // dos
		priority_announce("Simulations aborted, sensors report that the acasual event is normalizing. Good work, crew.","[command_name()] Higher Dimensional Affairs", '../assets/sound/announcer/notice/notice1.ogg')
		GLOB.cult_narsie = null
		addtimer(CALLBACK(GLOBAL_PROC, GLOBAL_PROC_REF(cult_ending_helper), CULT_FAILURE_NARSIE_KILLED), 2 SECONDS)
		return
...
/proc/narsie_apocalypse()
	if(QDELETED(GLOB.cult_narsie)) // tres
		priority_announce("Normalization detected! Abort the solution package!","[command_name()] Higher Dimensional Affairs", '../assets/sound/announcer/notice/notice1.ogg')
