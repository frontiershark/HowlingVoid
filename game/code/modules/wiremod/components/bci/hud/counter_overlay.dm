...
/obj/item/circuit_component/counter_overlay/input_received(datum/port/input/port)
	...
	var/image/counter = image(icon = '../assets/icons/hud/screen_bci.dmi', icon_state = "hud_numbers", loc = owner, layer = RIPPLE_LAYER)
	...
	for(var/i = 1 to 3)
		...
		var/image/number = image(icon = '../assets/icons/hud/screen_bci.dmi', icon_state = "hud_number_[cur_num]", loc = owner, layer = RIPPLE_LAYER)
