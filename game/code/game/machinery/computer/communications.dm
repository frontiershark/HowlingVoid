...
/obj/machinery/computer/communications/ui_act(action, list/params, datum/tgui/ui, datum/ui_state/ui_state)
	...
	switch (action)
		...
		if ("requestNukeCodes")
			...
			playsound(src, '../assets/sound/machines/terminal/terminal_prompt.ogg', 50, FALSE)
			COOLDOWN_START(src, important_action_cooldown, IMPORTANT_ACTION_COOLDOWN)
		...
		if ("toggleAuthentication")
			...
			if (authorize_name)
				...
				authorize_name = null
				playsound(src, '../assets/sound/machines/terminal/terminal_off.ogg', 50, FALSE)
				return
			...
			if (obj_flags & EMAGGED)
				...
				playsound(src, '../assets/sound/machines/terminal/terminal_alert.ogg', 25, FALSE)
			...
			state = STATE_MAIN
			playsound(src, '../assets/sound/machines/terminal/terminal_on.ogg', 50, FALSE)
