...
/datum/transport_controller/linear/proc/move_after_delay(lift_move_duration, door_duration, direction, mob/user)
	...
	playsound(prime_lift, '../assets/sound/vehicles/mecha/hydraulic.ogg', 25, vary = TRUE, frequency = clamp(HYDRAULIC_SFX_DURATION / lift_move_duration, 0.33, 3))
	...
	for(var/obj/machinery/door/elevator_door as anything in GLOB.elevator_doors)
		...
		switch(action)
			if(CYCLE_OPEN)
				...
				if(!played_ding)
					playsound(elevator_door, '../assets/sound/machines/ping.ogg', 50, TRUE)
					played_ding = TRUE
