...
/obj/structure/deployable_barricade/wirecutter_act(mob/living/user, obj/item/I)
	...
	playsound(src, '../assets/sound/items/tools/wirecutter.ogg', 25, TRUE)
...
/obj/structure/deployable_barricade/metal/click_alt(mob/user)
	if(portable_type)
		...
		if(do_after(user, 5 SECONDS, src))
			...
			playsound(src, '../assets/sound/items/tools/ratchet.ogg', 25, TRUE)
			fold_up()
...
/obj/structure/deployable_barricade/metal/proc/attempt_barricade_upgrade(obj/item/stack/sheet/iron/metal_sheets, mob/user, modifiers)
	...
	playsound(src, '../assets/sound/items/tools/screwdriver.ogg', 25, TRUE)
	update_icon()
...
/obj/structure/deployable_barricade/metal/welder_act(mob/living/user, obj/item/I)
	...
	playsound(src, '../assets/sound/items/tools/welder2.ogg', 25, TRUE)
...
/obj/structure/deployable_barricade/metal/crowbar_act(mob/living/user, obj/item/I)
	switch(build_state)
		if(BARRICADE_METAL_LOOSE) //Anchor bolts loosened step. Apply crowbar to unseat the panel and take apart the whole thing. Apply wrench to resecure anchor bolts
			...
			playsound(src, '../assets/sound/items/tools/crowbar.ogg', 25, 1)
			if(!do_after(user, 5 SECONDS, src))
				return TRUE

			user.visible_message(span_notice("[user] disassembles [src]."),
			span_notice("You disassemble [src]."))
			playsound(src, '../assets/sound/items/deconstruct.ogg', 25, 1)
			deconstruct(TRUE)
			return TRUE
...
/obj/structure/deployable_barricade/metal/plasteel/proc/toggle_open(state, mob/living/user)
	if(state == closed)
		return
	playsound(src, '../assets/sound/items/tools/ratchet.ogg', 25, 1)
	closed = !closed
	density = !density
